...
class WalletsDrawerHelper(WalletsDrawerHelperBase):
    ...
    def miscSetup(self) -> None:
        objs = NSBundle.mainBundle.loadNibNamed_owner_options_("WalletsCells",None,None)
        for obj in objs:
            if not isinstance(obj, UITableViewCell) and isinstance(obj, UIView) and obj.tag == 2000:
    ...
    def tableView_viewForHeaderInSection_(self, tableView, section) -> ObjCInstance:
        objs = NSBundle.mainBundle.loadNibNamed_owner_options_("WalletsCells",None,None)
        for obj in objs:
            if not isinstance(obj, UITableViewCell) and isinstance(obj, UIView) and obj.tag == 1000:
    ...
    def tableView_cellForRowAtIndexPath_(self, tableView, indexPath) -> ObjCInstance:
        ...
        if indexPath.row is not self.selectedRow:
            ...
            iv.image = UIImage.imageNamed_("bluechk.png")
